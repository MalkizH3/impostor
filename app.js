...
function wireEvents() {
  ...
  els.wordModeRandom.addEventListener("change", () => updateWordMode("random")); els.wordModeCustom.addEventListener("change", () => updateWordMode("custom")); els.anonymousVoteToggle.addEventListener("change", updateAnonymousVoting);
  els.customWordSaveBtn.addEventListener("click", submitCustomWord); els.customWordInput.addEventListener("keydown", (event) => { if (event.key === "Enter") submitCustomWord(); });
...
async function startGame() {
  ...
  const wordMode = state.room.wordMode || "random"; const turnOrder = [...players].sort(() => Math.random() - 0.5).map((player) => player.id); const roles = [...players].sort(() => Math.random() - 0.5).map((player, index) => ({ id: player.id, role: index === 0 ? "impostor" : "player" })); let index = 1;
  ...
  const customWordSetterId = wordMode === "custom" && nonImpostorIds.length ? nonImpostorIds[Math.floor(Math.random() * nonImpostorIds.length)] : null;
  const word = wordMode === "random" ? await randomWord() : null;
  await updateRoom({ status: "playing", players: players.map((player) => ({ ...player, ...byId[player.id], eliminated: false, association: "" })), word, round: 1, turnOrder, currentTurnId: wordMode === "custom" ? null : turnOrder[0], associations: {}, associationHistory: {}, votes: {}, guessVotes: {}, guess: null, winner: null, wordMode, customWordSetterId, awaitingWordSetup: wordMode === "custom" });
}
...
function render() {
  ...
  els.roomBadge.textContent = state.roomId || ""; els.players.innerHTML = (room.players || []).map((player) => `<div class="player-row"><strong>${escapeHtml(player.name)}</strong><span class="tag ${player.role || "pending"}">${roleLabel(player.role)}</span></div>`).join(""); const maxRoles = maxExtraRoles(room.players?.length || 0); const wordMode = room.wordMode || "random"; els.lobbyInfo.textContent = `${room.players?.length || 0} graczy. Host może uruchomić grę od 3 osób. Dodatkowe role: ${maxRoles === 0 ? "od 4 graczy" : maxRoles === 1 ? "maksymalnie 1" : "maksymalnie 2"}.`; els.start.classList.toggle("hidden", !isHost()); els.jester.checked = maxRoles > 0 && Boolean(room.options?.jester); els.executioner.checked = maxRoles > 0 && Boolean(room.options?.executioner); els.jesterChance.value = room.options?.jesterChance ?? 100; els.executionerChance.value = room.options?.executionerChance ?? 100; els.jester.disabled = !isHost() || maxRoles === 0; els.executioner.disabled = !isHost() || maxRoles === 0; els.jesterChance.disabled = !isHost() || maxRoles === 0; els.executionerChance.disabled = !isHost() || maxRoles === 0; els.wordModeRandom.checked = wordMode === "random"; els.wordModeCustom.checked = wordMode === "custom"; els.wordModeRandom.disabled = !isHost(); els.wordModeCustom.disabled = !isHost(); els.anonymousVoteToggle.checked = room.anonymousVoting !== false; els.anonymousVoteToggle.disabled = !isHost();
  if (room.status === "lobby") return; const me = currentPlayer(); const active = activePlayers(); const submitted = Object.keys(room.associations || {}).length; const turnPlayer = room.players?.find((player) => player.id === room.currentTurnId); const customWordWaiting = room.wordMode === "custom" && !room.word; const customSetter = room.players?.find((player) => player.id === room.customWordSetterId); const customWordAuthor = room.wordMode === "custom" && room.customWordSetterId ? room.players?.find((player) => player.id === room.customWordSetterId)?.name || "Ktoś" : null; els.round.textContent = room.round || 1; els.key.textContent = me ? roleDisplay(me, room) : "-";
  if (customWordWaiting) {
    ...
    els.status.textContent = me?.id === room.customWordSetterId ? "Wpisz hasło dla tej rundy." : `${escapeHtml(customSetter?.name || "Twórca hasła")} wpisuje hasło...`;
  } else {
    ...
    const authorLine = room.wordMode === "custom" && customWordAuthor ? `Autor hasła: ${escapeHtml(customWordAuthor)}.` : "";
    els.status.textContent = room.winner || room.lastVoteResult || (room.awaitingVote ? "Skojarzenia gotowe. Czas na głosowanie." : `${authorLine} Tura gracza: ${escapeHtml(turnPlayer?.name || "-")}.`.trim());
  }
  const allActiveVoted = active.length > 0 && Object.keys(room.votes || {}).length >= active.length; els.next.classList.toggle("hidden", !isHost() || !allActiveVoted); els.newGame.classList.toggle("hidden", !isHost() || room.status !== "finished"); els.end.classList.toggle("hidden", !isHost() || room.status === "finished"); els.table.innerHTML = (room.turnOrder || room.players?.map((player) => player.id) || []).map((playerId) => room.players.find((player) => player.id === playerId)).filter(Boolean).map((player) => playerCard(player, me, room)).join(""); bindDynamicControls();
  if (me?.role && room.word && !sessionStorage.getItem(`role_${state.roomId}_${me.id}`)) showRole(me, room);
  if (me?.id === room.customWordSetterId && room.wordMode === "custom" && !room.word) showWordSetupModal(); else hideWordSetupModal();
}
function playerCard(player, me, room) {
  ...
  const hasVoted = Object.prototype.hasOwnProperty.call(room.votes || {}, player.id) || Object.prototype.hasOwnProperty.call(room.guessVotes || {}, player.id);
  const voteVoters = Object.entries(room.votes || {}).filter(([, targetId]) => targetId === player.id).map(([voterId]) => room.players.find((entry) => entry.id === voterId)?.name || "Nieznany");
  const skipVoters = Object.entries(room.votes || {}).filter(([, targetId]) => targetId === "skip").map(([voterId]) => room.players.find((entry) => entry.id === voterId)?.name || "Nieznany");
  const active = activePlayers();
  const allActiveVoted = active.length > 0 && Object.keys(room.votes || {}).length >= active.length;
  const visibleVoteVoters = allActiveVoted ? voteVoters : [];
  const visibleSkipVoters = allActiveVoted ? skipVoters : [];
  const statusLabel = room.status === "finished" ? roleDisplay(player, room) : player.eliminated ? "Wyeliminowany" : association ? "Skojarzenie" : "Czeka"; const tileLabel = room.status === "finished" || player.eliminated ? `${statusLabel} · ${roleDisplay(player, room)}` : statusLabel;
  const finalRoleText = room.status === "finished" ? `<div class="final-role-inline">Rola: ${escapeHtml(roleLabel(player.role))}</div>` : "";
  const votedMarker = hasVoted ? '<div class="vote-status">Zagłosowano</div>' : "";
  const voteNames = room.anonymousVoting === false && visibleVoteVoters.length ? `<div class="vote-summary">Głosowali: ${escapeHtml(visibleVoteVoters.join(", "))}</div>` : "";
  const skipSummary = room.anonymousVoting === false && visibleSkipVoters.length ? `<div class="vote-summary skip-summary">Pominięcie: ${escapeHtml(visibleSkipVoters.join(", "))}</div>` : "";
  return `<article class="player-panel${player.eliminated ? " eliminated" : ""}${hasVoted ? " has-voted" : ""}" data-player-id="${player.id}"><div class="player-panel-head"><h3>${escapeHtml(player.name)}</h3><span class="badge">${escapeHtml(tileLabel)}</span></div>${votedMarker}${finalRoleText}<p class="player-report">${history.length ? escapeHtml(history.join("; ")) : "Jeszcze nic nie podał"}</p>${voteNames}${canAssociate ? '<div class="input-with-action"><input class="association-input" placeholder="Nowe skojarzenie" maxlength="80"><button class="btn primary association-btn">Podaj</button></div>' : ""}${canVote ? `<div class="button-row"><button class="btn vote-button" data-vote="${player.id}">Głosuj na ${escapeHtml(player.name)}</button><button class="btn vote-button" data-vote="skip">Pomiń głos</button></div>${skipSummary}` : ""}</article>`;
}
...
async function submitCustomWord() {
  const player = currentPlayer(); const room = state.room;
  if (!room || !player || player.id !== room.customWordSetterId) return toast("To nie jest twoja kolej, aby wpisać hasło.");
  const value = els.customWordInput.value.trim();
  if (!value) return toast("Wpisz hasło.");
  const firstTurnId = (room.turnOrder || []).find((id) => (room.players || []).some((entry) => entry.id === id));
  await updateRoom({ word: value, customWordSetterId: null, awaitingWordSetup: false, currentTurnId: firstTurnId || null });
  hideWordSetupModal();
}
