...
function wireEvents() {
  ...
  els.jester.addEventListener("change", () => updateOptions("jester")); els.executioner.addEventListener("change", () => updateOptions("executioner"));
...
async function updateOptions(changedRole) {
  ...
  await updateRoom({ options: { jester: els.jester.checked, executioner: els.executioner.checked } });
}
...
async function startGame() {
  ...
  const extraRoles = [state.room.options?.jester ? "jester" : null, state.room.options?.executioner ? "executioner" : null].filter(Boolean).slice(0, maxExtraRoles(players.length));
...
async function castGuessVote(agree) { const voter = currentPlayer(); if (!voter || voter.eliminated) return toast("Wyeliminowany gracz nie może głosować."); const votes = { ...(state.room.guessVotes || {}), [voter.id]: agree }; const active = activePlayers(); const updates = { guessVotes: votes }; if (Object.keys(votes).length >= active.length) { const accepted = Object.values(votes).filter(Boolean).length > active.length / 2; updates.winner = accepted && state.room.guess === state.room.word ? "Impostor odgadł hasło i wygrywa." : null; updates.guess = null; updates.guessVotes = {}; updates.awaitingGuessVote = false; } await updateRoom(updates); }
...
function showRole(player, room) {
  ...
  els.roleTitle.textContent = roleLabel(player.role);
  els.roleRole.textContent = roleLabel(player.role);
  ...
  els.roleVisual.textContent = player.role === "executioner"
    ? `Twój cel: ${room.players.find((item) => item.id === player.targetId)?.name || "brak"}`
    : "";
