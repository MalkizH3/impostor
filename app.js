...
function render() {
  ...
  els.winner.classList.toggle("hidden", !room.winner); els.winner.textContent = room.winner || ""; const allActiveVoted = active.length > 0 && Object.keys(room.votes || {}).length >= active.length; els.next.classList.toggle("hidden", !isHost() || !allActiveVoted); els.newGame.classList.toggle("hidden", !isHost() || room.status !== "finished"); els.end.classList.toggle("hidden", !isHost() || room.status === "finished"); els.table.innerHTML = (room.turnOrder || room.players?.map((player) => player.id) || []).map((playerId) => room.players.find((player) => player.id === playerId)).filter(Boolean).map((player) => playerCard(player, me, room)).join(""); bindDynamicControls();
